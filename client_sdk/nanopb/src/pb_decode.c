...
static bool checkreturn read_raw_value(pb_istream_t *stream, pb_wire_type_t wire_type, uint8_t *buf,
                                       size_t *size);
...
static const pb_decoder_t PB_DECODERS[PB_LTYPES_COUNT] = {
    &pb_dec_varint,  &pb_dec_uvarint, &pb_dec_svarint,    &pb_dec_fixed32,
    &pb_dec_fixed64,

    &pb_dec_bytes,   &pb_dec_string,  &pb_dec_submessage, NULL /* extensions */
};
...
bool checkreturn pb_decode_noinit(pb_istream_t *stream, const pb_field_t fields[],
  ...
  while (stream->bytes_left) {
    ...
    if (PB_HTYPE(iter.pos->type) == PB_HTYPE_REQUIRED &&
      ...
      fields_seen[iter.required_field_index >> 3] |=
          (uint8_t)(1 << (iter.required_field_index & 7));
    }
